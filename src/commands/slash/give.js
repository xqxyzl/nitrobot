...
module.exports = {
  ...
  .addStringOption(option =>
    ...
    run: async (client, interaction) => {
        ...
        await member.send(`${codeList}`).catch(e => {
            return interaction.reply({ content: `**Les __DM__ de __${member.user.username}__ sont fermés.**`, ephemeral: true })
        })
        await db.set(type, codes)
        return interaction.reply({ content: `**__${quantity}__ liens ${typee} ont été envoyés à __${member.user.username}__**`, ephemeral: true })        
    }
 };
