...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    run: async (client, interaction) => {
        ...
        if(await db.get(`owner.${member.user.id}`) === member.user.id) return interaction.reply({ content: `❌ **__${member.user.username}__ est déjà __owner__.**` })
        await db.set(`owner.${member.user.id}`, member.user.id)
         return interaction.reply({ content: `✅ **__${member.user.username}__ est maintenant __owner__.**` })
    }
 };
